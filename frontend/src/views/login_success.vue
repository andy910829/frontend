<template>
  ...
  <div class="body" @click="clicked">
    <div class="headertype">
      <pageheader @signOut="signOut()"/>
      <div class="server-time">伺服器時間:{{ this.time_text }}</div>
    </div>
    <group
      v-if="status === 'group'"
      @havegroup="status = 'ingroup'"
      @findgroup="status = 'findgroup'"
    />
...
export default {
  ...
  methods: {
    ...
    tTime() {
      ...
      console.log(this.ctTime+this.timerID)
  ...
  mounted() {
    ...
    this.timerID = window.setInterval(this.tTime, 1000);
  },
  created() {
    this.clicked();
    this.check();
