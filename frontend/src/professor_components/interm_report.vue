          ...
          <div class="card-left-components">
            ...
            <div class="left-score">
              {{ GroupInfo.leader.last_score + " "
                ...
                class="input-box"
                v-model="scoreResult[GroupInfo.leader.student_id]"
                @input="changeFromInput($event, GroupInfo.leader.student_id)"
              />
            </div>
            <div v-for="member in GroupInfo.member" class="left-score">
              ...
              <input
                class="input-box"
                v-model="scoreResult[member.student_id]"
                @input="changeFromInput($event, member.student_id)"
              />
            </div>
          </div>
          <div class="smbt-position">
            <el-button
            class="submit-bt"
...
export default {
  ...
  methods: {
    ...
    changeFromInput(event, name) {
      var score=Math.round(event.target.value)
      let res = {
    ...
    get_file(group) {
      ...
      axios
        ...
        .then((response) => {
          ...
          let file_name = group.group_id + "期中報告.pdf";
          if(response.data.size===0){
            this.$message.warning('尚未上傳期中報告')
          }
