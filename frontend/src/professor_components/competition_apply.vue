<template>
  <div>
    ...
    <div v-for="GroupInfo in GroupList" :key="GroupInfo.group_id">
      <div class="card">
        <li class="word">小組ID:{{ GroupInfo.group_id }}</li>
        <li class="word">
        ...
        <el-button
          type="success"
          class="ck-bt"
          @click="(this.ansStatus = 'accept'), ansCompetitonStatus(GroupInfo)"
          >接受</el-button
        >
        <el-button
          type="danger"
          class="ck-bt"
          @click="(this.ansStatus = 'reject'), ansCompetitonStatus(GroupInfo)"
          >拒絕</el-button
        >
      </div>
      <br />
  ...
  methods: {
    ...
    preview_file(group) {
      ...
      axios
        .post(
          path,
          { group_id: group.group_id, type: "competition_report",token:sessionStorage.getItem("token")},
          { responseType: "blob" }
        )
        .then((response) => {
    ...
    get_file(group) {
      ...
      axios
        ...
        .then((response) => {
          ...
          let file_name = group.group_id + "專題競賽報告.pdf";
