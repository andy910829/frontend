  ...
  <el-container>
    ...
    <el-main>
      ...
      <div v-else-if="status == 'accept' || status == 'waiting'" class="word">
        ...
        <li class="li-word">專題競賽題目:{{ this.detail.competition_topics }}</li>
        <li class="li-word">報名狀態:<span v-if="status === 'accept'">申請通過</span><span v-else-if="status === 'waiting'">待教授簽核中</span></li>
...
export default {
  ...
  methods: {
    ...
    cancel_reg() {
      ...
      axios
        .post(path, { group_id: sessionStorage.getItem("group_id") })
        .then((response) => {
